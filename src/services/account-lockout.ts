...
export async function recordFailedAttempt(
  ...
  console.log(
    `[AccountLockout] Failed attempt ${attemptCount} for ${email} from ${ipAddress}`
  );
...
async function lockAccount(
  ...
  try {
    ...
    console.log(
      `[AccountLockout] Account locked for ${email} until ${lockUntil}`
    );
...
export async function manualUnlock(
  ...
  try {
    ...
    console.log(
      `[AccountLockout] Account manually unlocked for ${email} by admin ${adminId}`
    );
  } catch (error) {
...
async function sendSecurityAlertEmail(
  ...
  try {
    if (alert.type === 'account_locked') {
      await sendEmail({
        ...
        data: {
          ...
          action:
            'If this was you, please wait for the lockout period to expire. If this was not you, please contact support immediately.',
    ...
    console.error(
      '[AccountLockout] Error sending security alert email:',
      error
    );
  }
}
