...
export async function updateSessionInfo(
  ...
  try {
    ...
    console.log(
      `[Session] Updated session info for user ${sessionInfo.userId}`
    );
  } catch (error) {
...
export async function getLocationFromIP(ipAddress: string): Promise<string> {
  ...
  try {
    const response = await fetch(
      `http://ip-api.com/json/${ipAddress}?fields=status,message,country,city,lat,lon`,
      {
        headers: {
 ...
 */
export async function getDetailedLocationFromIP(
  ipAddress: string
  ...
  try {
    const response = await fetch(
      `http://ip-api.com/json/${ipAddress}?fields=status,message,country,countryCode,city,lat,lon`,
      {
        headers: {
          'User-Agent': 'Saudi-Mais-Inventory-System',
        },
...
export async function terminateOtherSessions(
  ...
  try {
    ...
    console.log(
      `[Session] Terminated ${result.count} sessions for user ${userId}`
    );
    return result.count;
  } catch (error) {
...
export async function extendSession(
  ...
  try {
    ...
    console.log(
      `[Session] Extended session for user ${userId} to ${newExpiration.toISOString()}`
    );
    return true;
  } catch (error) {
