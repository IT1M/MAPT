...
export async function calculateSecurityScore(userId: string): Promise<SecurityScore> {
  ...
  if (hasStrongPassword && daysSincePasswordChange < 90) {
    ...
    recommendations.push('Change your password (last changed over 180 days ago)')
  } else {
  ...
  if (sessionCount === 0) {
    ...
    recommendations.push('You have multiple active sessions. Review and close unused sessions')
  } else {
    factors.activeSessions = 5
    recommendations.push('Too many active sessions detected. Close unused sessions immediately')
  }
  ...
  if (failedLogins === 0 && !accountLocked) {
    ...
    recommendations.push('Some failed login attempts detected. Monitor your account activity')
  } else if (accountLocked) {
    factors.recentSecurityEvents = 0
    recommendations.push('Your account was recently locked. Review security settings')
  } else {
    factors.recentSecurityEvents = 5
    recommendations.push('Multiple failed login attempts detected. Change your password if suspicious')
  }
...
export function getSecurityRecommendations(score: SecurityScore): {
  ...
  score.recommendations.forEach(rec => {
    if (rec.includes('immediately') || rec.includes('Enable two-factor')) {
      high.push(rec)
    } else if (rec.includes('Change your password') || rec.includes('Multiple failed')) {
      medium.push(rec)
    } else {
      low.push(rec)
    }
