    ...
    it('should validate multiple permissions', () => {
      const hasAllPermissions = (role: UserRole, permissions: string[]) => {
        return permissions.every((p) =>
          ROLE_PERMISSIONS[role].includes(p as any)
        );
      };

      expect(
        hasAllPermissions('ADMIN', ['inventory:read', 'inventory:write'])
      ).toBe(true);
      expect(
        hasAllPermissions('DATA_ENTRY', ['inventory:read', 'inventory:delete'])
      ).toBe(false);
    });
  });
});
