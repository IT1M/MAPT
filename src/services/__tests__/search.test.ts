...
describe('Search Service', () => {
  ...
  describe('searchSettings', () => {
    it('should return settings accessible to ADMIN', () => {
      ...
      expect(results.some((r) => r.title.includes('User Management'))).toBe(
        true
      );
    });
    ...
    it('should not return admin settings for DATA_ENTRY', () => {
      ...
      expect(results.every((r) => !r.title.includes('User Management'))).toBe(
        true
      );
      expect(results.every((r) => !r.title.includes('System Settings'))).toBe(
        true
      );
    });
    ...
    it('should return audit logs only for ADMIN and AUDITOR', () => {
      ...
      expect(dataEntryResults.some((r) => r.title.includes('Audit'))).toBe(
        false
      );
    });
