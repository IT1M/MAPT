...
class PerformanceMetricsService {
  ...
  recordAPIMetric(metric: Omit<APIMetric, 'timestamp'>): void {
    ...
    if (metric.status >= 400) {
      logger.error('API request error', new Error(metric.error || 'Unknown error'), {
        endpoint: metric.endpoint,
        method: metric.method,
        status: metric.status,
  ...
  getPerformanceStats(minutesAgo: number = 60): PerformanceStats {
    ...
    recentMetrics.filter(m => m.status >= 400).forEach(m => {
   ...
   */
  async checkAlertRules(): Promise<Array<{ rule: string; message: string; severity: 'warning' | 'critical' }>> {
    const alerts: Array<{ rule: string; message: string; severity: 'warning' | 'critical' }> = []
   ...
   */
  getEndpointMetrics(endpoint: string, minutesAgo: number = 60): {
    avgDuration: number
    p95: number
    errorRate: number
    requestCount: number
