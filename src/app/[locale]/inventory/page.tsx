...
export default function InventoryPage() {
  ...
  const [editingItem, setEditingItem] = useState<InventoryItemWithProduct | undefined>()
  ...
  const handleEditItem = (item: InventoryItemWithProduct) => {
    setEditingItem(item)
    setIsModalOpen(true)
  }
          ...
          <Select
            options={[
              ...
              ...ITEM_STATUS.map((status) => ({
                value: status,
                label: ITEM_STATUS_LABELS[status][locale as 'en' | 'ar'],
              })),
            ]}
            value={statusFilter}
                    ...
                    <tr key={item.id} className="hover:bg-gray-50 dark:hover:bg-gray-700">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <span className={`px-2 py-1 inline-flex text-xs leading-5 font-semibold rounded-full ${getStatusColor(item.status)}`}>
                          {ITEM_STATUS_LABELS[item.status][locale as 'en' | 'ar']}
                        </span>
                      </td>
                      {(canWrite || canDelete) && (
