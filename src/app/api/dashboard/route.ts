...
export async function GET(request: NextRequest) {
  try {
    ...
    const todayCount = inventoryItems.filter(
      (item) => item.createdAt >= todayStart
    ).length;
    const weekCount = inventoryItems.filter(
      (item) => item.createdAt >= weekStart
    ).length;
    const monthCount = inventoryItems.filter(
    ...
    const totalRejects = inventoryItems.reduce(
      (sum, item) => sum + item.reject,
      0
    );
    const rejectRate =
      totalQuantity > 0 ? (totalRejects / totalQuantity) * 100 : 0;
    ...
    const maisItems = inventoryItems.filter(
      (item) => item.destination === 'MAIS'
    );
    const fozanItems = inventoryItems.filter(
      (item) => item.destination === 'FOZAN'
    );
    const maisPercentage =
      totalItems > 0 ? (maisItems.length / totalItems) * 100 : 0;
    const fozanPercentage =
      totalItems > 0 ? (fozanItems.length / totalItems) * 100 : 0;
