  ...
  try {
    ...
    if (!format || !['csv', 'excel', 'json', 'pdf'].includes(format)) {
      return validationError(
        'Invalid format. Must be csv, excel, json, or pdf'
      );
    }
    ...
    if (
      !entity ||
      !['inventory', 'audit', 'reports', 'users'].includes(entity)
    ) {
      return validationError(
        'Invalid entity. Must be inventory, audit, reports, or users'
      );
    }
    ...
    switch (format) {
      ...
      case 'pdf':
        ...
        return validationError(
          'PDF export should use entity-specific endpoints'
        );
      default:
