...
export async function GET(request: NextRequest) {
  try {
    ...
    const timeSeriesData = new Map<string, { accepted: number; rejected: number }>()

    inventoryItems.forEach((item) => {
      const dateKey = formatDate(item.createdAt)
      const existing = timeSeriesData.get(dateKey) || { accepted: 0, rejected: 0 }
    ...
    const totalAccepted = inventoryItems.reduce((sum, item) => sum + (item.quantity - item.reject), 0)
    const totalRejected = inventoryItems.reduce((sum, item) => sum + item.reject, 0)
    ...
    if (timeSeries.length >= 4) {
      ...
      const firstHalfAvg = firstHalf.reduce((sum, item) => sum + item.rejectRate, 0) / firstHalf.length
      const secondHalfAvg = secondHalf.reduce((sum, item) => sum + item.rejectRate, 0) / secondHalf.length
