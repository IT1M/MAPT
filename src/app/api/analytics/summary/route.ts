...
export async function GET(request: NextRequest) {
  try {
    ...
    const totalReject = inventoryItems.reduce(
      (sum, item) => sum + item.reject,
      0
    );
    const rejectRate =
      totalQuantity > 0 ? (totalReject / totalQuantity) * 100 : 0;
    ...
    const uniqueUserIds = new Set(
      inventoryItems.map((item) => item.enteredById)
    );
    ...
    if (userContributions.size > 0) {
      const topUserId = Array.from(userContributions.entries()).sort(
        (a, b) => b[1] - a[1]
      )[0];
