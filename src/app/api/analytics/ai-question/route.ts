...
export async function POST(request: NextRequest) {
  try {
    ...
    const qaResponse = await geminiService.askQuestion(
      question,
      inventoryContext
    );
  ...
  // Low stock questions
    ...
    if (context.lowStockItems && context.lowStockItems.length > 0) {
      const itemsList = context.lowStockItems
        ...
        .map(
          (item: any) =>
            `${item.itemName} (${item.currentStock}/${item.reorderPoint})`
        )
        .join(', ');
  ...
  // Recent activity questions
    ...
    if (context.recentActivity && context.recentActivity.length > 0) {
      const activityList = context.recentActivity
        ...
        .map(
          (item: any) =>
            `${item.itemName} (${item.quantity} units to ${item.destination})`
        )
        .join(', ');
