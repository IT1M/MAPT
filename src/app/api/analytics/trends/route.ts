  ...
  try {
    ...
    if (!validPeriods.includes(period)) {
      return validationError(
        'Invalid period. Must be one of: 7d, 30d, 90d, 1y'
      );
    }
    ...
    inventoryItems.forEach((item) => {
      ...
      timeSeriesData.set(dateKey, {
        ...
        maisQuantity:
          existing.maisQuantity +
          (item.destination === 'MAIS' ? item.quantity : 0),
        fozanQuantity:
          existing.fozanQuantity +
          (item.destination === 'FOZAN' ? item.quantity : 0),
