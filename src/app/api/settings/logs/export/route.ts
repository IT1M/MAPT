...
export async function GET(request: NextRequest) {
  try {
    ...
    auditLogs.forEach((log, index) => {
      ...
      logLines.push(`User: ${log.user.name} (${log.user.email}) - Role: ${log.user.role}`)
      logLines.push(`Action: ${log.action} on ${log.entityType}${log.entityId ? ` (ID: ${log.entityId})` : ''}`)
    ...
    systemSettings.forEach((setting) => {
      ...
      const isSensitive = sensitiveKeys.some((key) => setting.key.toLowerCase().includes(key))
      const value = isSensitive ? '***MASKED***' : JSON.stringify(setting.value)
