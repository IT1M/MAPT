...
> = {
  ...
  default_destination: {
    type: 'string',
    validate: (v) => !v || ['MAIS', 'FOZAN'].includes(v),
  },
  ...
  default_report_format: {
    type: 'string',
    validate: (v) => ['PDF', 'EXCEL', 'CSV'].includes(v),
  },
  ...
  max_items_per_user_per_day: {
    type: 'number',
    validate: (v) => v >= 1 && v <= 10000,
  },
  ...
  log_level: {
    type: 'string',
    validate: (v) => ['error', 'warning', 'info', 'debug'].includes(v),
  },
    ...
    const updatedSettings = await prisma.$transaction(async (tx) => {
      ...
      for (const setting of settings) {
        ...
        if (setting.key.startsWith('app_')) category = 'general';
          ...
          setting.key.includes('stock') ||
          setting.key.includes('batch') ||
          setting.key.includes('destination')
        )
          category = 'inventory';
        else if (
          setting.key.includes('session') ||
          setting.key.includes('login') ||
          setting.key.includes('password') ||
          setting.key.includes('2fa')
        )
          category = 'security';
        else if (
          ...
          setting.key.includes('rate_limit') ||
          setting.key.includes('cache') ||
          setting.key.includes('gemini')
        )
          category = 'api';
