...
export async function GET(request: NextRequest) {
  try {
    ...
    const destinations =
      destinationsParam.length > 0
        ? (destinationsParam.filter(
            (d) => d === 'MAIS' || d === 'FOZAN'
          ) as Destination[])
        : undefined;
    ...
    const totalQuantity = aggregateItems.reduce(
      (sum, item) => sum + item.quantity,
      0
    );
    const totalRejects = aggregateItems.reduce(
