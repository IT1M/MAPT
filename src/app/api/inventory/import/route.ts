...
import type { ImportOptions, ValidationError, ImportResult } from '@/types/import'
...
export async function POST(request: NextRequest) {
  try {
    ...
    const options: ImportOptions = optionsStr ? JSON.parse(optionsStr) : { duplicateHandling: 'skip' }
    ...
    if (rows.length > 10000) {
      return validationError('Maximum 10,000 items per import. Please split your file.')
    }
        ...
        if (options.duplicateHandling !== 'create') {
          ...
          if (existing) {
            if (options.duplicateHandling === 'skip') {
              duplicates.push(`${validationResult.data.itemName} - ${validationResult.data.batch}`)
              continue
