...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (
      body.scheduleTime &&
      !/^([0-1][0-9]|2[0-3]):[0-5][0-9]$/.test(body.scheduleTime)
    ) {
      return validationError(
        'Invalid schedule time format. Must be HH:mm (e.g., 02:00)'
      );
    }
    ...
    if (body.formats) {
      ...
      if (
        ...
        !body.formats.every((f: string) => validFormats.includes(f))
      ) {
        return validationError(
          'Invalid formats. Must be an array of CSV, JSON, or SQL'
        );
      }
    }
    ...
    if (
      body.retentionDailyDays !== undefined &&
      (body.retentionDailyDays < 1 || body.retentionDailyDays > 365)
    ) {
      return validationError('Daily retention must be between 1 and 365 days');
    }

    if (
      body.retentionWeeklyWeeks !== undefined &&
      (body.retentionWeeklyWeeks < 1 || body.retentionWeeklyWeeks > 52)
    ) {
      return validationError('Weekly retention must be between 1 and 52 weeks');
    }

    if (
      body.retentionMonthlyMonths !== undefined &&
      (body.retentionMonthlyMonths < 1 || body.retentionMonthlyMonths > 60)
    ) {
      return validationError(
        'Monthly retention must be between 1 and 60 months'
      );
    }
    ...
    if (body.scheduleTime !== undefined)
      updateData.scheduleTime = body.scheduleTime;
    if (body.formats !== undefined) updateData.formats = body.formats;
    if (body.includeAuditLogs !== undefined)
      updateData.includeAuditLogs = body.includeAuditLogs;
    if (body.retentionDailyDays !== undefined)
      updateData.retentionDailyDays = body.retentionDailyDays;
    if (body.retentionWeeklyWeeks !== undefined)
      updateData.retentionWeeklyWeeks = body.retentionWeeklyWeeks;
    if (body.retentionMonthlyMonths !== undefined)
      updateData.retentionMonthlyMonths = body.retentionMonthlyMonths;
