...
export function DashboardExporterWrapper() {
  const [dashboardData, setDashboardData] = useState<DashboardSnapshot | null>(
    null
  );
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      if (!summaryResult.success) {
        throw new Error(
          summaryResult.error?.message || 'Failed to fetch summary'
        );
      }
      ...
      const mostActiveCategory =
        summaryData.byCategory?.[0]?.category || undefined;
