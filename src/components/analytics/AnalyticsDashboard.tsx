...
export const AnalyticsDashboard: React.FC = () => {
  ...
  const buildQueryString = useCallback(
    (filterState: AnalyticsFilterState): string => {
  ...
  const fetchSummary = useCallback(
    async (filterState: AnalyticsFilterState) => {
      try {
  ...
  const updateCategoryData = useCallback((summaryResult: any) => {
    if (summaryResult?.byCategory) {
      const catData: CategoryDataPoint[] = summaryResult.byCategory.map(
        (cat: any) => ({
  ...
  const fetchRejectAnalysis = useCallback(
    async (filterState: AnalyticsFilterState) => {
      try {
        ...
        const response = await fetch(
          `/api/analytics/reject-analysis?${queryString}`
        );
        ...
        if (result.success && result.data?.timeSeries) {
          setData((prev) => ({
            ...prev,
            rejectAnalysis: result.data.timeSeries,
          }));
        }
      } catch (err) {
        ...
        setErrors((prev) => ({
          ...prev,
          rejectAnalysis: err instanceof Error ? err.message : 'Unknown error',
        }));
      } finally {
  ...
  const fetchUserActivity = useCallback(
    async (filterState: AnalyticsFilterState) => {
      ...
      try {
        ...
        const queryString = buildQueryString(filterState);
        const response = await fetch(
          `/api/analytics/user-activity?${queryString}`
        );
        ...
        setErrors((prev) => ({
          ...prev,
          userActivity: err instanceof Error ? err.message : 'Unknown error',
        }));
      } finally {
        setLoading((prev) => ({ ...prev, userActivity: false }));
      }
  ...
  const updateMonthlyComparison = useCallback(
    (trendsData: TrendDataPoint[]) => {
      ...
      trendsData.forEach((point) => {
        ...
        monthData.items += 1;
        monthData.rejectRate =
          (point.rejectQuantity / point.totalQuantity) * 100;
      });
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const response = await fetch(
          '/api/inventory/data-log?page=1&pageSize=1000'
        );
        if (response.ok) {
          const result = await response.json();
          if (result.success && result.data) {
            const uniqueCategories = Array.from(
              new Set(
                result.data.map((item: any) => item.category).filter(Boolean)
              )
            ) as string[];
  ...
  const handleTrendDataPointClick = useCallback(
    (date: string) => {
      const locale =
