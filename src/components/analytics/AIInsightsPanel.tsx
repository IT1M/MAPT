...
export function AIInsightsPanel({
      ...
      if (data.success) {
        ...
        if (data.data.insights) {
          data.data.insights.forEach((insight: any) => {
            parsedInsights.push({
              type: insight.type === 'warning' ? 'alert' : insight.type === 'info' ? 'finding' : 'recommendation',
              priority: insight.priority,
              title: insight.type === 'warning' ? 'Alert' : insight.type === 'info' ? 'Finding' : 'Recommendation',
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md border border-gray-200 dark:border-gray-700" dir={locale === 'ar' ? 'rtl' : 'ltr'}>
      {/* Header */}
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {groupedInsights.recommendations.map((insight, index) => (
                      <InsightCard key={`recommendation-${index}`} insight={insight} />
                    ))}
                  </div>
                </div>
              )}
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {groupedInsights.predictions.map((insight, index) => (
                      <InsightCard key={`prediction-${index}`} insight={insight} />
                    ))}
                  </div>
                </div>
              )}
