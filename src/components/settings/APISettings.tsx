...
export function APISettings() {
  ...
  const [geminiConfig, setGeminiConfig] = useState<GeminiConfiguration | null>(
    null
  );
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const config: GeminiConfiguration = {
        apiKey:
          apiSettings.find((s: any) => s.key === 'gemini_api_key')?.value || '',
        model:
          apiSettings.find((s: any) => s.key === 'gemini_model')?.value ||
          'gemini-pro',
        temperature: parseFloat(
          apiSettings.find((s: any) => s.key === 'gemini_temperature')?.value ||
            '0.7'
        ),
        maxTokens: parseInt(
          apiSettings.find((s: any) => s.key === 'gemini_max_tokens')?.value ||
            '2048'
        ),
        cacheInsightsDuration: parseInt(
          apiSettings.find((s: any) => s.key === 'gemini_cache_duration')
            ?.value || '30'
        ),
        features: {
          insights:
            apiSettings.find((s: any) => s.key === 'ai_insights_enabled')
              ?.value === true,
          predictiveAnalytics:
            apiSettings.find((s: any) => s.key === 'ai_predictive_enabled')
              ?.value === true,
          naturalLanguageQueries:
            apiSettings.find((s: any) => s.key === 'ai_nlq_enabled')?.value ===
            true,
        },
      };
    ...
    try {
      ...
      if (updates.maxTokens !== undefined) {
        settingsToUpdate.push({
          ...
          value: updates.maxTokens,
        });
      }
      if (updates.cacheInsightsDuration !== undefined) {
        settingsToUpdate.push({
          key: 'gemini_cache_duration',
          value: updates.cacheInsightsDuration,
        });
      }
      if (updates.features !== undefined) {
        settingsToUpdate.push({
          key: 'ai_insights_enabled',
          value: updates.features.insights,
        });
        settingsToUpdate.push({
          key: 'ai_predictive_enabled',
          value: updates.features.predictiveAnalytics,
        });
        settingsToUpdate.push({
          key: 'ai_nlq_enabled',
          value: updates.features.naturalLanguageQueries,
        });
      }
