...
export function UserModal({ user, isOpen, onClose, onSubmit }: UserModalProps) {
  ...
  const roleOptions = [
    { value: UserRole.DATA_ENTRY, label: 'Data Entry - Can add and edit inventory items' },
    { value: UserRole.SUPERVISOR, label: 'Supervisor - Can manage inventory and view analytics' },
    { value: UserRole.MANAGER, label: 'Manager - Can view analytics and generate reports' },
    { value: UserRole.AUDITOR, label: 'Auditor - Can view audit logs and reports' },
    { value: UserRole.ADMIN, label: 'Admin - Full system access' },
  ]

  return (
    <Modal isOpen={isOpen} onClose={onClose} title={isEditMode ? 'Edit User' : 'Create New User'} size="medium">
        ...
        {!isEditMode && (
          <div className="flex items-center gap-2">
            <input
              ...
              onChange={(e) =>
                setFormData((prev) => ({ ...prev, sendWelcomeEmail: e.target.checked }))
              }
              className="h-4 w-4 rounded border-gray-300 text-primary-600 focus:ring-primary-500"
            />
