...
describe('InventoryTable', () => {
  ...
  it('hides columns on mobile', () => {
    ...
    const hiddenOnMobile = container.querySelectorAll('.hidden.md\\:table-cell')
    expect(hiddenOnMobile.length).toBeGreaterThan(0)
  })

  it('renders column customization button', () => {
    render(<InventoryTable items={mockItems} onToggleColumnVisibility={vi.fn()} />)
  ...
  it('respects column visibility settings', () => {
    ...
    render(<InventoryTable items={mockItems} columnVisibility={columnVisibility} />)
  ...
  it('applies custom column widths', () => {
    ...
    const itemNameHeader = Array.from(headers).find(h => h.textContent?.includes('Item Name'))
  ...
  it('shows column menu when column settings button is clicked', async () => {
    ...
    const { findByText } = render(<InventoryTable items={mockItems} onToggleColumnVisibility={onToggle} />)
  ...
  it('calls onRowClick when Enter key is pressed', () => {
    const onRowClick = vi.fn()
    const { container } = render(<InventoryTable items={mockItems} onRowClick={onRowClick} />)
  ...
  describe('Virtual Scrolling', () => {
    it('uses virtual scrolling for datasets with more than 1000 rows', () => {
      ...
      const largeDataset: InventoryItemWithUser[] = Array.from({ length: 1500 }, (_, i) => ({
        id: `${i + 1}`,
        itemName: `Item ${i + 1}`,
        batch: `BATCH-${i + 1}`,
        quantity: 100,
      ...
      expect(screen.getByText(/Using virtual scrolling for 1,500 items/)).toBeInTheDocument()
    })

    it('does not use virtual scrolling for datasets with 1000 or fewer rows', () => {
      // Create a dataset with exactly 1000 items
      const mediumDataset: InventoryItemWithUser[] = Array.from({ length: 1000 }, (_, i) => ({
        id: `${i + 1}`,
        itemName: `Item ${i + 1}`,
        batch: `BATCH-${i + 1}`,
        quantity: 100,
      ...
      expect(screen.queryByText(/Using virtual scrolling/)).not.toBeInTheDocument()
