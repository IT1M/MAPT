...
export function Breadcrumbs({
  ...
  const breadcrumbItems =
    items || generateBreadcrumbsFromPath(pathname, locale, t, isMounted);
  ...
  const orderedItems =
    locale === 'ar' ? [...displayItems].reverse() : displayItems;
 ...
 */
function getSegmentLabel(segment: string, t: any): string {
...
function truncateBreadcrumbs(
  ...
  return [items[0], { label: '...', path: undefined }, ...items.slice(-2)];
}
...
export function BreadcrumbsWithBack({
  ...
  return (
    <div className={`flex items-center gap-3 ${className}`}>
      ...
      >
        <ChevronRight
          className={`w-5 h-5 text-gray-500 dark:text-gray-400 ${locale === 'ar' ? '' : 'rotate-180'}`}
        />
      </button>
