...
export const productSchema = z.object({
  ...
  .refine((data) => data.reorderPoint >= data.minStockLevel && data.reorderPoint <= data.maxStockLevel, {
...
export const inventoryItemSchema = z.object({
  ...
  batch: z
    .string()
    .regex(/^[A-Za-z0-9-]+$/, 'Batch must contain only alphanumeric characters and hyphens')
...
export const registrationSchema = z.object({
  ...
  role: z.enum(['ADMIN', 'DATA_ENTRY', 'SUPERVISOR', 'MANAGER', 'AUDITOR']).optional().default('DATA_ENTRY'),
})
...
export const auditLogQuerySchema = z.object({
  ...
  action: z.enum(['CREATE', 'UPDATE', 'DELETE', 'LOGIN', 'LOGOUT', 'EXPORT']).optional(),
