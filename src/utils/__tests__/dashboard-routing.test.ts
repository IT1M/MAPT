...
describe('Dashboard Routing', () => {
  ...
  describe('getDashboardPath', () => {
    ...
    it('should use callback URL when provided and valid', () => {
      ...
      expect(getDashboardPath('ADMIN' as UserRole, callbackUrl)).toBe(
        '/inventory'
      );
    });
  ...
  describe('getDashboardUrl', () => {
    it('should prepend locale to dashboard path', () => {
      ...
      expect(getDashboardUrl('MANAGER' as UserRole, 'ar')).toBe(
        '/ar/analytics'
      );
    });
    ...
    it('should not duplicate locale if already present', () => {
      const callbackUrl = '/inventory';
      expect(getDashboardUrl('ADMIN' as UserRole, 'en', callbackUrl)).toBe(
        '/inventory'
      );
    });
    ...
    it('should handle callback URL with locale prepending', () => {
      const callbackUrl = '/inventory';
      expect(getDashboardUrl('ADMIN' as UserRole, 'ar', callbackUrl)).toBe(
        '/ar/inventory'
      );
    });
  });
