...
export function validateFormAccessibility(form: HTMLFormElement): {
  ...
  inputs.forEach((input) => {
    ...
    if (!label && !ariaLabel && !ariaLabelledby) {
      issues.push(`Input "${htmlInput.name || htmlInput.type}" is missing a label`)
    }
  })
  ...
  requiredInputs.forEach((input) => {
    ...
    if (!htmlInput.hasAttribute('aria-required')) {
      issues.push(`Required input "${htmlInput.name}" is missing aria-required attribute`)
    }
  })
  ...
  errorMessages.forEach((error) => {
    ...
    const associatedInput = form.querySelector(`[aria-describedby="${describedBy}"]`)
    
    if (!associatedInput) {
      issues.push(`Error message "${htmlError.textContent}" is not associated with any input`)
    }
  })
