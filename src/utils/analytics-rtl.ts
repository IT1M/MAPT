 ...
 */
export function getIconTransform(locale: string, iconType: 'arrow' | 'chevron' | 'trend'): string {
  const rtl = isRTL(locale)
...
export function getSpacingClassRTL(
  ...
  const actualSide = rtl ? (side === 'left' ? 'r' : 'l') : (side === 'left' ? 'l' : 'r')
 ...
 */
export function formatChartNumber(value: number, locale: string, options?: Intl.NumberFormatOptions): string {
 ...
 */
export function formatChartDate(date: Date | string, locale: string, format: 'short' | 'medium' | 'long' = 'short'): string {
 ...
 */
export function getBarChartOrientation(locale: string, categoryCount: number): 'horizontal' | 'vertical' {
