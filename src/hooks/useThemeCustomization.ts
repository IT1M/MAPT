...
export function useThemeCustomization() {
  ...
  const updateFonts = useCallback((fonts: Partial<Theme['fonts']>) => {
    const newTheme = {
      ...currentTheme,
      fonts: {
        ...currentTheme.fonts,
  ...
  const updateDensity = useCallback((density: Theme['spacing']['density']) => {
    const newTheme = {
      ...currentTheme,
      spacing: {
  ...
  const updateAnimations = useCallback((animations: Partial<Theme['animations']>) => {
    const newTheme = {
      ...currentTheme,
      animations: {
  ...
  const importTheme = useCallback((themeJson: string): { success: boolean; error?: string } => {
    try {
      ...
      if (!theme.id || !theme.name || !theme.colors || !theme.fonts || !theme.spacing || !theme.animations) {
  ...
  const importFromShareCode = useCallback((shareCode: string): { success: boolean; error?: string } => {
    try {
