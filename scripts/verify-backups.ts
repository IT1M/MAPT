  ...
  try {
    ...
    if (backup.validated) {
      addResult({
        ...
        details: backup.validatedAt
          ? `Last validated: ${backup.validatedAt.toISOString()}`
          : undefined,
      });
    } else {
      addResult({
...
async function verifyRecentBackups(): Promise<boolean> {
  ...
  try {
    ...
    if (recentAppBackup) {
      const hoursAgo =
        (Date.now() - recentAppBackup.createdAt.getTime()) / (1000 * 60 * 60);
      addResult({
    ...
    const backupDir =
      process.env.BACKUP_DIR || '/var/backups/mais-inventory/database';
    ...
    if (fs.existsSync(backupDir)) {
      ...
      if (files.length > 0) {
        ...
        const hoursAgo =
          (Date.now() - latestBackup.mtime.getTime()) / (1000 * 60 * 60);
