...
import { ROUTE_PERMISSIONS, type UserRole } from '../src/utils/route-permissions'
...
async function verifyRouteProtection() {
  ...
  navRoutes.forEach(route => {
    ...
    check(
      `Navigation route "${route}" has permission config`,
      hasPermission
    )
  })
  ...
  Object.entries(ROUTE_PERMISSIONS).forEach(([route, roles]) => {
    check(
      `Route "${route}" has at least one role`,
      roles.length > 0
    )
  })
  ...
  Object.entries(criticalRoutes).forEach(([route, expectedRoles]) => {
    ...
    check(
      `Critical route "${route}" has correct restrictions`,
      rolesMatch
    )
  ...
  const validRoles: UserRole[] = ['ADMIN', 'DATA_ENTRY', 'SUPERVISOR', 'MANAGER', 'AUDITOR']
  
  Object.entries(ROUTE_PERMISSIONS).forEach(([route, roles]) => {
    const allValid = roles.every(role => validRoles.includes(role))
    check(
      `Route "${route}" has only valid roles`,
      allValid
    )
  ...
  Object.entries(roleAccessCount).forEach(([role, count]) => {
    ...
    log(`  ${role}: ${count}/${permissionRoutes.length} routes (${percentage}%)`, 'blue')
  })
  ...
  if (passedChecks === totalChecks) {
    ...
    log('⚠️  Some route protection checks failed. Please review the output above.', 'yellow')
  }
